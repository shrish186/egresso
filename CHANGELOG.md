...
- Claude Code PreToolUse hook (`egresso init` / `doctor`).
- Generic MCP proxy (`egresso proxy -- <server>`) for any MCP client.
- Audit log (`.egresso/audit.jsonl`) with `egresso log` / `report`.
